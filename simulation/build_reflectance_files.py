...
def build_geographic(dimensions, output_directory, spectra_starting_col, normalize, sensor, level):
    ...
    for continent in sorted(df.CONTINENT.unique()):
        if continent in ['Australia', 'Asia']:
            continue

        # filter out current continent, current continent is in simulation; this gets passed to pc analysis
        df_to_unmix = df_norm.loc[(df_norm['CONTINENT'] != continent)].copy().reset_index(drop=True)
        df_to_unmix_soils = df_to_unmix.loc[(df_to_unmix['level_1'] == 'soil')].copy().reset_index(drop=True)
        df_to_unmix_veg = df_to_unmix.loc[(df_to_unmix['level_1'] != 'soil')].copy().reset_index(drop=True)

        # soils for simulation
        df_to_sim = df_norm.loc[(df_norm['CONTINENT'] == continent)].copy().reset_index(drop=True)

        # run soil PCA;
        pc_components = spectra.pca_analysis(df_to_unmix, spectra_starting_col=spectra_starting_col)
        ...
        df_ch = df_to_unmix_soils.iloc[hull_indices]
        ...
        spectra.df_to_envi(df=df_to_sim, spectral_starting_column=spectra_starting_col, wvls=wvls,
        ...
        spectra.simulate_reflectance(df_sim=df_to_sim, df_unmix=df_unmix, dimensions=dimensions,
...
def get_library_outputs(output_directory):
    ...
    synthetic_output = os.path.join(output_directory, 'synthetic_images')
    create_directory(em_lib_output)
    create_directory(sim_lib_output)
    create_directory(synthetic_output)
...
def build_hull(dimensions: int, output_directory:str,  spectra_starting_col:int, spectral_bundles:int, geo_filter,
    ...
    # merge the em dataframes
    df_unmix = pd.concat([unmix_npv_pv, df_ch], axis=0).sort_values(level)

    # save the dataframes to a csv - unmixing library
    df_unmix.to_csv(os.path.join(em_libraries_output, f'convex_hull__n_dims_{dimensions}_sensor_{sensor}_geofilter_{str(geo_filter)}_unmix_library.csv'), index=False)
    df_sim = pd.concat([df, df_unmix]).drop_duplicates(keep=False).sort_values(level)
    ...
    # save unmix library as envi
    spectra.df_to_envi(df=df_unmix, spectral_starting_column=spectra_starting_col, wvls=wvls,
                       output_raster=os.path.join(em_libraries_output, f'convex_hull__n_dims_{dimensions}_sensor_{sensor}_geofilter_{str(geo_filter)}_unmix_library.hdr'))

    # save simulation library as envi
    spectra.df_to_envi(df=df_sim, spectral_starting_column=spectra_starting_col, wvls=wvls,
                       output_raster=os.path.join(sim_libraries_output, f'convex_hull__n_dims_{dimensions}_sensor_{sensor}_geofilter_{str(geo_filter)}_simulation_library.hdr'))

    # # simulate the reflectance
    spectra.simulate_reflectance(df_sim=df_sim, df_unmix=df_unmix, dimensions=dimensions,
                                 sim_libraries_output=sim_libraries_output,
                                 name=f'convex_hull__n_dims_{dimensions}_sensor_{sensor}_geofilter_{str(geo_filter)}',
                                 level=level, spectral_bundles=spectral_bundles, cols=cols,
                                 output_directory=synthetic_output, wvls=wvls,
                                 spectra_starting_col=spectra_starting_col)
