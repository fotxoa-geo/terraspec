        ...
        if atmospheric_rtm == "libradtran":
            ...
            if atmospheric_rtm == "sRTMnet":
                ...
                for path in ["emulator_aux_file", "emulator_file",
                             "earth_sun_distance_file", "irradiance_file", 'engine_base_dir', 'aerosol_template_path']:
...
#
def main():
    parser = argparse.ArgumentParser(description='Run hypertrace')
    parser.add_argument('-wvls', '--wavelength_file', type=str, help='Specify instrument wavelengths', default='emit')
    parser.add_argument('-refl', '--reflectance_file', type=str, help='Specify reflectance')
    parser.add_argument('-rtm', '--rtm_template_file', type=str, help='Specify rtm_template_file')
    parser.add_argument('-lutd', '--lutdir', type=str, help='Specify look up table directory')
    parser.add_argument('-od', '--outdir', type=str, help='Specify out directory')
    parser.add_argument('-surface', '--surface_file', type=str, help='Specify surface reflectance file that contains priors')
    parser.add_argument('-noise', '--noise_file', type=str, help='Specify noise file', default=None)
    parser.add_argument('-snr', '--signal_noise_ratio', type=int, help='signal to noise ratio', default=300)
    parser.add_argument('-aod', '--aerial_optical_depth', type=float, help='Aerial optical depth (0 - 1)', default=0.1)
    parser.add_argument('-h2o', '--water_content_atmosphere', type=float, help='Water Content Atmosphere (0 - 5)', default=1.0)
    parser.add_argument('-type', '--atmosphere_type', type=str, help='Atmosphere Type', default='ATM_MIDLAT_WINTER')
    parser.add_argument('-sza', '--sensor_zenith_angle', type=float, help='sensor zenith angle (feeds into solar zenith in hypertrace)', default=0)
    parser.add_argument('-oza', '--observer_zenith_angle', type=float,help='observer zenith angle', default=0)
    parser.add_argument('-saa', '--solar_azimuth_angle', type=float, help='solar azimuth angle', default=0)
    parser.add_argument('-oaa', '--observer_azimuth_angle', type=float, help='observer azimuth angle', default=0)
    parser.add_argument('-oa', '--observer_altitude_km', type=float, help='observer altitude in km', default=99.9)
    parser.add_argument('-doy', '--day_of_year', type=int, help='Day of year', default=200)
    parser.add_argument('-lat', '--latitude', type=float, help='Latitude', default=34.15)
    parser.add_argument('-long', '--longitude', type=float, help='Longitude', default=118.14)
    parser.add_argument('-time', '--local_time', type=float, help='Solar Zenith Angle in Decinam Hours', default=20)
    parser.add_argument('-elev', '--elevation', type=float, help='Elevation above sea level', default=0.01)
    parser.add_argument('-inv_mode', '--inversion_mode', type=str, help='Specify inversion mode', default='inversion')
    parser.add_argument('-empirical', '--use_empirical_line', type=bool, help='Use empirical line on big files', default=False)
    parser.add_argument('-calib_uncer', '--calibration_uncertainty_file', type=str, help='Calibration uncertainty file', default=None)
    parser.add_argument('-calib_draws', '--calibration_draws', type=int, help='Number of calibration draws', default=1)
    parser.add_argument('-calib_scale', '--calibration_scale', type=int, help='Calibration scale', default=1)
    parser.add_argument('-lut', '--create_lut', type=bool, help='Create look up table', default=True)
    parser.add_argument('-ovwr', '--overwrite', type=bool, help='Overwrite', default=True)
    parser.add_argument('-configs', '--configuration_file', type=str, help='Configuration file')

    # load arguments
    args = parser.parse_args()
    # load isofit configuration
    ...
    do_hypertrace(isofit_config=isofit_config, wavelength_file=args.wavelength_file,
                  reflectance_file=args.reflectance_file, rtm_template_file=args.rtm_template_file, lutdir=args.lutdir,
                  outdir=args.outdir, surface_file=args.surface_file, noisefile=args.noise_file,
                  snr=args.signal_noise_ratio, aod=args.aerial_optical_depth, h2o=args.water_content_atmosphere, 
                  atmosphere_type=args.atmosphere_type,solar_zenith=args.sensor_zenith_angle, observer_zenith=args.observer_zenith_angle,
                  solar_azimuth=args.solar_azimuth_angle, observer_azimuth=args.observer_azimuth_angle,
                  observer_altitude_km=args.observer_altitude_km, dayofyear=args.day_of_year, latitude=args.latitude,
                  longitude=args.longitude, localtime=args.local_time, elevation_km=args.elevation,
                  inversion_mode=args.inversion_mode, use_empirical_line=args.use_empirical_line,
