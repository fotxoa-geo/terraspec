...
def decode_expert_system(tetra_expert_file, groups=DEFAULT_GROUPS, log_file=DEFAULT_LOGFILE,
    ...
    while expert_line_index < len(expert_file_text):
        ...
        valid_feature_constraints = ['ct', 'lct', 'rct', 'lct/rct>', 'rct/lct>', 'rcbblc>', 'rcbblc<', 'lcbbrc>',
                                     'lcbbrc<', 'r*bd>']
        if 'define features' in expert_file_text[expert_line_index]:
            ...
            while ('endfeatures' not in expert_file_text[expert_line_index]):
                toks = expert_file_text[expert_line_index].strip().split()
                if len(toks) > 5 and toks[0].startswith('f') and (
                        toks[1] == 'DLw' or toks[1] == 'MLw' or toks[1] == 'OLw'):
                    local_feature = {}
                    ...
                    for _t in range(len(toks) - 1, 5, -1):
                        if '\#' in toks[_t]:
                            ...
                            local_feature[toks[_t]] = recast_globals(
                                [toks[_local_tok] for _local_tok in range(_t + 1, last_valid)], globals)
                            last_valid = _t

                    features.append(local_feature)
                expert_line_index += 1

        valid_constituent_constraints = ['FIT', 'FITALL', 'DEPTH', 'DEPTHALL', 'DEPTH-FIT', 'FD', 'FDALL', 'FD-FIT',
                                         'FD-DEPTH']
