...
def main():
    ...
    parser.add_argument('--reference_library', '-slib', type=str,
                        default='Spectral-Library-Reader-master/s06av18a_envi', metavar='REFERENCE_LIBRARY')
    parser.add_argument('--research_library', '-rlib', type=str, default='Spectral-Library-Reader-master/r06av18a_envi',
                        metavar='RESEARCH_LIBRARY')
    ...
    if args.calculate_uncertainty == 1:
        ...
        emit_utils.file_checks.check_raster_files([args.reflectance_file, args.reflectance_uncertainty_file],
                                                  map_space=False)
    ...
    for key, item in spectral_reference_library_files.items():
        ...
        for _f, (filename, library_name, record) in enumerate(
                zip(unique_file_names, library_names.tolist(), records.tolist())):
            if library_name == key:
    ...
    for _c, constituent_file in enumerate(unique_file_names):
        ...
        if args.calculate_uncertainty and np.sum(library_normalized_band_depth != 0) > 0:
            mixture_uncertainty = calculate_uncertainty(ref_lib['wavelengths'], observed_reflectance,
                                                        ...
                                                        ref_lib['reflectance'][
                                                        ref_lib['library_records'].index(records[_c]), :],
                                                        decoded_expert[filepath_to_key(constituent_file)]['features'][
                                                            0]['continuum'])
...
def calculate_band_depth(wavelengths: np.array, reflectance: np.array, feature: tuple, record: int = None,
                         name: str = None):
    ...
    if record is not None and name is not None:
        ...
        plt.plot([wavelengths[feature_inds][bd_ind], wavelengths[feature_inds][bd_ind]],
                 [reflectance[feature_inds][bd_ind], continuum[feature_inds][bd_ind]])
        plt.title(f'{record}   |||   {name}\nBD = {max(depths)}')
        plt.savefig(f'/beegfs/scratch/brodrick/emit/reflectance_analyses/figs/{record}_{name.replace("/", "-")}.png',
                    dpi=200, bbox_inches='tight')

    return max(depths)


def calculate_uncertainty(wavelengths: np.array, observed_reflectance: np.array,
                          observed_reflectance_uncertainty: np.array,
    ...
    num = (len(inds) * np.sum(observed_reflectance[:, inds, :] * library_reflectance[np.newaxis, inds, np.newaxis],
                              axis=1) - \
    ...
    left_term = np.power(1 / (
                np.sum(np.power(a[:, np.newaxis, :] * library_reflectance[np.newaxis, inds, np.newaxis], 2),
                       axis=1) - np.power(
            np.sum(a[:, np.newaxis, :] * library_reflectance[np.newaxis, inds, np.newaxis], axis=1), 2)), 2)
    ...
    right_term = np.sum(np.power(
        len(inds) * a[:, np.newaxis, :] * library_reflectance[np.newaxis, inds, np.newaxis] - np.sum(
            a[:, np.newaxis, :] * library_reflectance[np.newaxis, inds, np.newaxis], axis=1)[:, np.newaxis, :],
        2) * np.power(observed_reflectance_uncertainty[:, inds, :], 2), axis=1)
...
def unique_file_fractions(fraction_dict: OrderedDict, decoded_expert: OrderedDict):
    ...
    for key, item in fraction_dict.items():
        for constituent in item:
            ...
            logging.debug(
                f'file: {unique_file_names[idx]}, DN_scale: {scaling[idx]}, library: {library[-1]}, record: {record[idx]}')
